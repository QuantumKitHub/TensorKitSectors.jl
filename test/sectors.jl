...
using LinearAlgebra
using TensorKitSectors: dim_from_Fsymbol, twist_from_Rsymbol, FS_indicator_from_Fsymbol, FS_phase_from_Fsymbol,
    Asymbol_from_Fsymbol, Bsymbol_from_Fsymbol, Fsymbol_from_fusiontensor
...
@testsuite "Fallback definitions" I -> begin
    for a in smallset(I)
        @test dim(a) ≈ dim_from_Fsymbol(a)
        @test frobenius_schur_phase(a) ≈ FS_phase_from_Fsymbol(a)
        @test frobenius_schur_indicator(a) == FS_indicator_from_Fsymbol(a)
        BraidingStyle(I) isa HasBraiding && @test twist(a) ≈ twist_from_Rsymbol(a)
        for b in smallset(I), c in smallset(I)
            @test Asymbol(a, b, c) ≈ Asymbol_from_Fsymbol(a, b, c)
            @test Bsymbol(a, b, c) ≈ Bsymbol_from_Fsymbol(a, b, c)
        end
    end
        ...
        for a in smallset(I), b in smallset(I), c in smallset(I)
            for e in ⊗(a, b), f in ⊗(b, c)
                for d in intersect(⊗(e, c), ⊗(a, f))
                    ...
                    F2 = Fsymbol_from_fusiontensor(a, b, c, d, e, f)
                    @test isapprox(F1, F2; atol = 1.0e-12, rtol = 1.0e-12)
                end
            end
        end
